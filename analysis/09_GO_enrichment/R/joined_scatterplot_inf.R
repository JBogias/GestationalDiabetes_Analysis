joined_scatterplot_inf <- function(modules, ont) {
  ...
  for(i in inner_modules) {
    tryCatch({sim_matrix_BP <- read_rds(
      here(paste0("data/GO_analysis/inf/", ont, "/", i, "_", ont, "_simMatrix.rds"))
    )
    
    reduced_terms_BP <- read_rds(
      here(
        paste0("data/GO_analysis/inf/", ont, "/", i, "_", ont, "_reducedTerms.rds")
        )
    )
