...
# 2. Load Data
Through initial PCA already performed I've managed to identify a sample that has poor sequencing. There are also four other samples which have funky read distributions on the transcript level, however I will err on the side of keeping them in for now. All five are commented out, only 1173 will be removed.
...
# Load Metadata
Load in the patient data (pd) and define the model matrix formula here to be used in the workflow
```{r metadata}
